...
// modular arithmetic
function method sub_byte(x : byte, y : byte) : byte
{
...
function method byte_from(x : int) : byte
{
  (x % 256) as byte
}

// uint32 as sequence of 4 bytes
function method unpack(x : uint32) : seq<byte>
  ensures |unpack(x)| == 4;
{
...
// 4 bytes as uint32
function method pack(x : seq<byte>) : uint32
  requires |x| == 4;
{
